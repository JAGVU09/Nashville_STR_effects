...
grouped_viol<-Violations %>% 
  ...
  tally() %>% 
  mutate(Violations_per_dist = n)
```

```{r}

bins <- c(0,50,100,200,400,800,1000,2000,4000,8000)
pal <- colorBin("YlOrRd", domain = grouped_STRs$STRs_per_dist, bins = bins)

m<- leaflet(data = STRs, options = leafletOptions(minZoom = 0, maxZoom = 20)) %>% 
  addTiles() %>% addProviderTiles(providers$Stamen.TonerLite,
                                  options = providerTileOptions(noWrap = TRUE)) %>% 
  addPolygons(data = council_districts,
              fillColor = ~pal(grouped_STRs$STRs_per_dist),
              weight = 2,
              opacity = 1,
              color = "white",
              fillOpacity = 0.7)
