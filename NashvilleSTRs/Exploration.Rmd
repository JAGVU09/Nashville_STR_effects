...
```{r}
STRs<- geojson_list(STRs, lat = 'mapped_location.latitude', lon = 'mapped_location.longitude')
```
...
```{r}
STRs <- STRs %>% 
  mutate_at(.vars = c(mapped_location, mapped_location), ~as.numeric(.))
```
```{r}
STRs$mapped_location
```



```{r}
m <- leaflet() %>% setView(lng = -86.785014, lat = 36.163371, zoom = 11) %>% 
  addTiles() %>% 
  addMarkers(STRs)
m
```
