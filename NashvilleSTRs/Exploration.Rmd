...
query = list(
  '$select'= 'request,date_received,property_apn,property_address,reported_problem,council_district,mapped_location, zip, census_tract',
  '$limit' = 90000
)
...
Violmap<- leaflet(data = STRs_Viol_per_district, options = leafletOptions(minZoom = 0, maxZoom = 20)) %>% 
  addTiles() %>% addProviderTiles(providers$Stamen.TonerLite,
                                  options = providerTileOptions(noWrap = TRUE)) %>% 
  addPolygons(data = council_districts_geo,
              fillColor = ~pal(STRs_Viol_per_district$Violations_per_dist),
              opacity = 0.2,
              dashArray = "3",
              fillOpacity = 0.9,
              highlightOptions = highlightOptions(
...
1. Maybe calculate violations per STR? 
